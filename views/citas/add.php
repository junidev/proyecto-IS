...
if(!$_SESSION["usuario"]){
	...
	if($_SESSION['rol']==2){
 	...
 	$medicos = MedicosController::combo_medicos(); 
          	    ...
          	    <form class="form-horizontal" method="post" action="agregar_cita" name="add_cita" id="add_cita" novalidate="novalidate">
                 	<div class="control-group">
			              <label class="control-label">Medicos</label>
			               <div class="controls">
				                <select  style="width:220px;" name="medico_id" class="roles" >
				                	<?php foreach ($medicos as $key => $value): ?>
				                			  <option value="<?php echo $value['id']; ?>"><?php echo $value['nombres'].' - '.$value['apellido_paterno'].' - '.$value['especialidad']; ?></option>
				                	<?php endforeach ?>
	               ...
	               <div class="control-group">
			              <label class="control-label">Pacientes</label>
			               <div class="controls">
				                <select  style="width:220px;" name="paciente_id" class="roles" >
				                	<?php foreach ($pacientes as $key => $value): ?>
				                			  <option value="<?php echo $value['id']; ?>"><?php echo $value['nombres'].' - '.$value['apellido_paterno']; ?></option>
				                	<?php endforeach ?>
